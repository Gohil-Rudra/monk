...
func Eval(node ast.Node) object.Object {
	...
	case *ast.ReturnStatement:
		val := Eval(node.ReturnValue) // return ni andar nu expression decode kariye
...
func evalStatements(stmts []ast.Statement) object.Object {
	...
	for _, statement := range stmts {
		...
		if returnObj, ok := result.(*object.ReturnValue); ok {
			return returnObj.Value // here its Value is an Object like our result as return is just a warapper around an object , so check if the object as return wrapper and if it does unwrap it and return that object as result.
		}
	}
